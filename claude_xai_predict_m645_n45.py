...
────────────────────────────────
Du doan M645:
  - 45 model nhi phan cho N1-N6 (6 so chinh, range 1-45) → chon top 8

Kien truc:
  - 1 XGBoost/LightGBM/CatBoost binary classifier / so (45 model)
  - Walk-forward CV voi precision metric
  - No data leak: tach train (ky 1→N-1) va predict_row (ky N)
  - Sliding window + Time decay weight
  - Ensemble 5 configs (M1/M2/M3/M4/M5)
...
Usage:
    ...
    python claude_xai_predict_m645_n45.py --source db --top 8 --ensemble --filter 0.13
    python claude_xai_predict_m645_n45.py --source db --top 8 --ensemble --filter 0.13 --save
...
VERSION     = "v1.0.0"
DESCRIPTION = (
    "45 binary XGB/LGBM/CatBoost (n1-n6), no-leak train/predict split, "
    "sliding window + time decay, PW cross features, "
    "walk-forward CV precision, ensemble 5 configs, save to m645kqpredict"
)
...
def get_drawn_set(row) -> set:
    """Lay tap hop n1-n6 (so chinh) tu mot row M645."""
    return {
...
def make_global_features(df: pd.DataFrame, lags: int = 3) -> pd.DataFrame:
    ...
    prev_cols = [
        ...
        "dec0","dec1","dec2","dec3","dec4",   # M645 chi co dec0-dec4
    ]
...
def make_per_number_features(df: pd.DataFrame, num: int) -> pd.DataFrame:
    """Tinh features rieng cho so `num` trong N1-N6 (range 1-45)."""
    ...
    feat["n_dec"]     = float(num // 10)   # 0-4 cho M645
...
def make_xgb(params: dict = None):
    defaults = dict(
        ...
        reg_alpha             = 0.05,
        reg_lambda            = 1.0,
        scale_pos_weight      = 6,   # 45/6 ~ 7.5, dung 6
...
def train_all(df: pd.DataFrame, global_feat: pd.DataFrame,
    ...
    """Train 45 model cho so chinh N1-N6."""
    results  = {}
    baseline = 6 / 45   # = 13.3%

    print(f"\n  [N1-N6] Baseline precision (random 8/45): {baseline:.1%}")
...
def predict_top_k_ensemble(all_results: list,
                            top_k: int = 8,
                            filter_threshold: float = 0.13) -> list:
    """Average xac suat, filter, chon top_k."""
    proba = []
    for num in ALL_NUMS:
        probs, cvps = [], []
...
def main():
    parser = argparse.ArgumentParser(description="XGBoost predict M645 — 45 so chinh")
    ...
    parser.add_argument("--filter",       type=float, default=0.13)
    ...
    if args.ensemble:
        print(f"\n🚀 Training ensemble ({len(ENSEMBLE_CONFIGS)} configs × 45 so)...")
