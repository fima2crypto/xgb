"""
claude_fima_xgboost_m645_n45.py
────────────────────────────────
Du doan M645: 45 model nhi phan cho N1-N6 (range 1-45), chon top 8.
Khong co so dac biet/bonus.

Kien truc (theo format L535/P655):
  - 1 XGBoost binary classifier / so (45 model)
  ...
  - Update result → tu dong lay tu m645kqdetail

Cross-ref: chi dung Power P655 (bo Lotto do it overlap).

Usage:
    python claude_fima_xgboost_m645_n45.py --source db --lags 3 --top 8
    python claude_fima_xgboost_m645_n45.py --source db --top 8 --ensemble --filter 0.12
    python claude_fima_xgboost_m645_n45.py --source db --top 8 --ensemble --filter 0.12 --save
...
ALL_NUMS = list(range(1, 46))   # Mega: so 1-45

# ── Version ───────────────────────────────────────────────────────────────────
VERSION     = "v1.0.0"
DESCRIPTION = (
    "45 binary XGBoost (n1-n6, range 1-45), no-leak train/predict split, "
    "sliding window + time decay, Power cross features, "
    "walk-forward CV precision, ensemble 3 configs, save to m645kqpredict"
)
...
def get_drawn_set(row) -> set:
    """Lay tap hop N1-N6 tu mot row M645."""
    return {
...
def make_global_features(df: pd.DataFrame, lags: int = 3) -> pd.DataFrame:
    ...
    Tat ca shift >= 1. Cross-ref: chi Power P655.
    """
    d = df.copy()
    ...
    # E. Power features (no shift — pw* la ky Power TRUOC ky Mega)
    for c in ["pw_thu_enc","pw_ngay_diff","pw_dd","pw_sc"]:
        if c in d.columns:
            feat[c] = d[c]

    # F. Cross Power x Mega
...
def make_per_number_features(df: pd.DataFrame, num: int) -> pd.DataFrame:
    ...
    streak, cur_streak = [], 0
    for i, h in enumerate(hit):
        streak.append(np.nan if i == 0 else float(cur_streak))
    ...
    # n_pair_freq: tan suat xuat hien cung so ky truoc (30 ky)
    ...
    feat["n_dec"]     = float(num // 10)   # 0-4 cho Mega 1-45
...
def make_xgb(params: dict = None):
    defaults = dict(
        ...
        reg_alpha             = 0.05,
        reg_lambda            = 1.0,
        scale_pos_weight      = 6,        # Mega: ~6/45 ~ 13% positive
...
def train_all(df: pd.DataFrame, global_feat: pd.DataFrame,
    ...
    results  = {}
    baseline = 6 * 8 / 45 / 8   # = 13.3%

    print(f"\n  Baseline precision (random 8/45): {baseline:.1%}")
    if train_window > 0:
        print(f"  Sliding window : {train_window} ky gan nhat")
    print(f"  Decay weight   : {decay}")
    ...
    print(f"  So beat baseline  : {sum(1 for p in precs if p > baseline)}/45")
...
def _show_top_importance(results: dict, top_n: int = 15):
    ...
    imp_avg = {k: v/45 for k, v in imp_sum.items()}
    top = sorted(imp_avg.items(), key=lambda x: -x[1])[:top_n]
    print(f"\n  Top {top_n} features (trung binh 45 model):")
    for f, s in top:
...
ENSEMBLE_CONFIGS = [
    ...
    {"lags":3, "decay":0.995, "train_window":400, "label":"M3(lag3,d0.995,w400)"},
]
...
def predict_top_k_ensemble(all_results: list, top_k: int = 8,
    ...
    for num in ALL_NUMS:
        ...
        proba_list.append((num, float(np.mean(probs)), float(np.mean(cvps))))

    if filter_threshold > 0:
        before     = len(proba_list)
        proba_list = [(n,p,c) for n,p,c in proba_list if c >= filter_threshold]
        removed    = before - len(proba_list)
        if removed > 0:
            print(f"\n  🔍 Filter {filter_threshold:.0%}: loai {removed} so, con {len(proba_list)} so")
...
def predict_top_k_single(results: dict, top_k: int = 8,
    ...
    for num in ALL_NUMS:
        ...
        proba_list.append((num, prob, r["prec"]))
...
def show_prediction(df: pd.DataFrame, proba_list: list,
    ...
    print(f"🎯 Du doan {top_k} so cho ky (sau {last_ky}):")
    print(f"{'═'*58}")
...
def save_predict(args, df: pd.DataFrame, proba_list: list, cv_prec_mean: float):
    import psycopg2
    ...
    try:
        ...
        conn.commit()
        print(f"\n💾 Da luu du doan vao m645kqpredict"
...
def main():
    parser = argparse.ArgumentParser(description="XGBoost predict M645 — 45 so")
    ...
    parser.add_argument("--val_size",     type=int,   default=30,
                        help="Walk-forward val_size (mac dinh 30 cho M645)")
    ...
    if args.ensemble:
        print(f"\n🚀 Training ensemble ({len(ENSEMBLE_CONFIGS)} models × 45 so)...")
        ...
        print(f"\n🚀 Training 45 model (top={args.top})...")
        results = train_all(
            df, global_feat,
        ...
        proba_list   = predict_top_k_single(
